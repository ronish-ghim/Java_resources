...
`windowActivated()`, `windowDeactivated()`, `windowOpened()`, `windowClosed()`, `windowClosing()`, `windowIconified()` and `windowDeiconified()`.
...
| WindowEvent | WindowListener | windowActivated(), windowDeactivated(), windowOpened(), windowClosed(), windowClosing(), windowIconified() and windowDeiconified(). | Window events occur when a window's status is changed. |
